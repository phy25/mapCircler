		...
		var change = function(e){ //当选择一个文件的时候（事件 onchange）
			...
			reader.onload = function(e){
				if(e.target.readyState == 2){
					jsonObj = $.evalJSON(e.target.result);
					deleteCurrentRegion(map);
					currentRegionOrder = getRegionOrder(jsonObj, jsonObj.currentID);
					createRegion(map, jsonObj, currentRegionOrder, true);
				}else{ // 出错
...
function saveFile(){ // text, filename	
	if($.browser.chrome){
		...
		var $h = $('<div style="position:absolute;z-index:10;top:0;left:0;width:50px;height:30px;"></div>'), $a = $('<a />').attr('href', 'data:text/octet-stream,' + encodeURIComponent($.toJSON(jsonObj))).attr('download', filename+'.mcf').text('点击').click(function(){$h.remove();});
		$h.append($a).appendTo('body');
		//download 属性定义下载的文件名
	}else{
		...
		$h.appendTo('body').find('textarea').text($.toJSON(jsonObj)).focus();
	}
}
...
function editBlock(event) {
    ...
    $('#item_name').val(jsonObj.regions[currentRegionOrder].elements[elementIndex].name);
    $('#item_meta').val(jsonObj.regions[currentRegionOrder].elements[elementIndex].info);
	$("#info").show();
